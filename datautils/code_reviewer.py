...
class SimpleGenDataset(TextDataset):
    def __init__(self, tokenizer, pool, args, file_path, samplenum=-1):
        ...
        if os.path.exists(savep):
            ...
            logger.info("Reading examples from {}".format(file_path))
            data = read_jsonl_code_reviewer(file_path)
...
def read_jsonl_code_reviewer(path):
    data = []
    with open(path) as f:
        for line in f:
            try:
...
class ReviewExample(object):
    ...
    @classmethod
    def from_json(cls, js, maxl: int=200):
        """class method to directly create ReviewExample object from json dict."""
        return cls(
            idx=js["idx"],
            oldf=js["oldf"],
            diff=js["patch"],
